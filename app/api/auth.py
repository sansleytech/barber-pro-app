...
def _armar_respuesta_login(db: Session, usuario: Usuario, barberia: Barberia):
    """Arma la misma respuesta de siempre (token + datos del usuario),
    reutilizada tanto por el login directo como por la verificación de código."""
    token = crear_token({
...
def login(
    ...
    Si el usuario tiene email cargado, primero se manda un código de
    verificación por correo antes de entregar el token de sesión.
    """
    ...
    if usuario.email:
        ...
        usuario.codigo_verificacion_expira = datetime.now(timezone.utc) + timedelta(minutes=10)
        db.commit()

        from app.core.email import enviar_email
        enviar_email(
            destinatario=usuario.email,
            asunto="Tu código de verificación — Barber Pro",
            cuerpo_html=f"""
                <p>Hola {usuario.nombre_usuario},</p>
                <p>Tu código de verificación es:</p>
                <h2 style="letter-spacing: 4px;">{codigo}</h2>
                <p>Vence en 10 minutos. Si no fuiste vos, ignorá este correo.</p>
            """,
        )
        return {"requiere_verificacion": True, "id_usuario": usuario.id_usuario}

    # Sin email cargado, no podemos mandar código: entra directo (compatibilidad).
...
def verificar_codigo(datos: VerificarCodigo, db: Session = Depends(get_db)):
    """Segundo paso del login: valida el código que llegó por email y,
    si es correcto, entrega el token de sesión."""
    usuario = db.query(Usuario).filter(Usuario.id_usuario == datos.id_usuario).first()
    if usuario is None or not usuario.codigo_verificacion:
    ...
    if not expira or expira < datetime.now(timezone.utc):
        raise HTTPException(status_code=400, detail="El código venció, iniciá sesión de nuevo")

    if datos.codigo.strip() != usuario.codigo_verificacion:
