...
def crear(datos: ValoracionCrear, db: Session = Depends(get_db),
          id_barberia: int = Depends(get_barberia_actual)):
    turno = db.query(Turno).filter(
        Turno.id_turno == datos.id_turno, Turno.id_barberia == id_barberia
    ).first()
    if turno is None:
    ...
    db.add(valoracion); db.commit(); db.refresh(valoracion)
    return valoracion
