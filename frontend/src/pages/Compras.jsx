...
import {
    Plus, Search, ShoppingBag, Wallet, Package, X, Save, Calendar,
} from "lucide-react";
...
function Compras() {
    ...
    const filtradas = compras.filter((c) => {
        const texto = `${nombreProducto(c.id_producto)} ${nombreProveedor(c.id_proveedor)} ${c.factura || ""}`.toLowerCase();
        return texto.includes(busqueda.toLowerCase());
    });

    // Métricas del mes actual
    const mesActual = hoyStr.slice(0, 7); // "2026-06"
    const comprasMes = compras.filter((c) => c.fecha_compra?.slice(0, 7) === mesActual);
    const totalMes = comprasMes.reduce((acc, c) => acc + Number(c.costo_total), 0);
    const unidadesMes = comprasMes.reduce((acc, c) => acc + c.cantidad, 0);
    const totalHistorico = compras.reduce((acc, c) => acc + Number(c.costo_total), 0);

    const metricas = [
        { titulo: "COMPRAS DEL MES", valor: comprasMes.length, sub: "este mes", icono: ShoppingBag, color: "text-white", borde: "border-line" },
        { titulo: "INVERTIDO ESTE MES", valor: formatoPrecio(totalMes), sub: "costo total", icono: Wallet, color: "text-gold", borde: "border-gold/30" },
        { titulo: "UNIDADES", valor: unidadesMes, sub: "compradas este mes", icono: Package, color: "text-white", borde: "border-line" },
        { titulo: "TOTAL HISTÓRICO", valor: formatoPrecio(totalHistorico), sub: "todas las compras", icono: Wallet, color: "text-white", borde: "border-line" },
    ];
    ...
    const columnas = [
        ...
        },
        {
            campo: "id_producto",
            titulo: "Producto",
            render: (c) => <span className="text-white font-medium">{nombreProducto(c.id_producto)}</span>,
        },
        {
            campo: "id_proveedor",
            titulo: "Proveedor",
        ...
        },
        {
            ...
            titulo: "Total",
            alinear: "right",
            render: (c) => <span className="text-gold font-semibold">{formatoPrecio(c.costo_total)}</span>,
        },
    ];
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {metricas.map((m) => (
                    <div key={m.titulo} className={`bg-ink-card border ${m.borde} rounded-2xl p-5`}>
                        <div className="flex items-center gap-2 text-xs font-medium text-gray-400 mb-3">
                            <m.icono className="w-4 h-4" />
                            {m.titulo}
                        </div>
                        <div className={`text-3xl font-bold mb-1 ${m.color}`}>{m.valor}</div>
            ...
            {cargando ? (
                <div className="text-center py-16 text-gray-500">Cargando compras...</div>
            ) : error ? (
                ...
                <Tabla
                    columnas={columnas}
                    datos={filtradas}
                    vacioTexto={compras.length === 0 ? "Sin compras todavía. Registrá la primera con \"Nueva compra\"" : "No hay compras con esa búsqueda"}
                />
            )}
                ...
                <>
                    ...
                    <div className="fixed top-0 right-0 h-full w-full max-w-md bg-ink-card border-l border-line z-50 overflow-y-auto">
                        ...
                        <form onSubmit={registrar} className="p-5 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm text-gray-300 mb-1.5">Producto *</label>
                                <select
                            ...
                            <div>
                                <label className="block text-sm text-gray-300 mb-1.5">Proveedor</label>
                                <select
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        inputMode="numeric"
                                        value={form.cantidad}
                                        onChange={(e) => cambiar("cantidad", soloNumeros(e.target.value))}
                                        placeholder="0"
                                ...
                                <div>
                                    <label className="block text-sm text-gray-300 mb-1.5">Costo unitario *</label>
                                    <input
                                        type="text"
                                        inputMode="numeric"
                                        value={form.costo_unitario}
                                        onChange={(e) => cambiar("costo_unitario", soloNumeros(e.target.value))}
                                        placeholder="0"
                                        className="w-full bg-ink border border-line rounded-lg px-4 py-2.5 text-white placeholder-gray-600 focus:outline-none focus:border-gold"
                                        required
                                    />
                            ...
                            <div className="flex items-center justify-between bg-ink rounded-lg border border-line px-4 py-3">
                                ...
                                <span className="text-gold font-bold text-lg">{formatoPrecio(costoTotal)}</span>
                            </div>

                            <div>
                                <label className="block text-sm text-gray-300 mb-1.5">Fecha de compra *</label>
                                <input
                                    type="date"
                            ...
                            <div>
                                <label className="block text-sm text-gray-300 mb-1.5">N° de factura</label>
                                <input
                                    type="text"
                                    value={form.factura}
