...
function Servicios() {
    ...
    const filtrados = servicios.filter((s) =>
        `${s.nombre} ${s.descripcion || ""}`.toLowerCase().includes(busqueda.toLowerCase())
    );
    ...
    const columnas = [
        ...
        },
        {
            campo: "precio",
            titulo: "Precio",
            render: (s) => <span className="text-gold font-medium">{formatoPrecio(s.precio)}</span>,
        },
        {
        ...
        <div>
            ...
            {cargando ? (
                ...
                <Tabla
                    columnas={columnas}
                    datos={filtrados}
                    vacioTexto={busqueda ? "No se encontraron servicios" : "Todavía no hay servicios"}
                />
            )}
        </div>
    );
