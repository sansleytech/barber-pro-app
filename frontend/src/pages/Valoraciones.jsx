...
function Valoraciones() {
    ...
    const [form, setForm] = useState({ id_turno: "", estrellas: 0, comentario: "" });
    ...
    const promedioGeneral =
        cantidad > 0
            ? (valoraciones.reduce((acc, v) => acc + v.estrellas, 0) / cantidad).toFixed(1)
            : "0.0";
        ...
        <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-gray-400">Reseñas y calificaciones de los servicios</p>
                </div>
                <button
                    onClick={abrirPanel}
            ...
            {cargando ? (
                <div className="text-center py-16 text-gray-500">Cargando valoraciones...</div>
            ) : (
                <>
                    {/* Resumen */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                        <div className="bg-ink-card border border-gold/30 rounded-2xl p-5">
                            <div className="text-xs font-medium text-gray-400 mb-2">PROMEDIO GENERAL</div>
                            <div className="flex items-center gap-3">
                                <span className="text-3xl font-bold text-gold">{promedioGeneral}</span>
                                <Estrellas valor={Math.round(promedioGeneral)} size="w-5 h-5" />
                            </div>
                        </div>
                        <div className="bg-ink-card border border-line rounded-2xl p-5">
                            <div className="text-xs font-medium text-gray-400 mb-2">TOTAL RESEÑAS</div>
                            <div className="text-3xl font-bold text-white">{cantidad}</div>
                        </div>
                        <div className="bg-ink-card border border-line rounded-2xl p-5">
                            <div className="text-xs font-medium text-gray-400 mb-2">SIN VALORAR</div>
                            <div className="text-3xl font-bold text-white">{turnosSinValorar.length}</div>
                            <div className="text-xs text-gray-500 mt-1">turnos completados</div>
                        </div>
                    </div>
                    ...
                    {rankingBarberos.length > 0 && (
                        <div className="bg-ink-card border border-line rounded-2xl p-6 mb-6">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h2 className="text-white font-semibold">Ranking de barberos</h2>
                            </div>
                            <div className="space-y-3">
                                {rankingBarberos.map((b, i) => (
                                    <div key={b.id} className="flex items-center gap-3">
                                        <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold ${i === 0 ? "bg-gold text-ink" : "bg-ink text-gray-400"
                                            }`}>
                                            {i + 1}
                                        </span>
                                        <span className="text-white font-medium flex-1">{b.nombre}</span>
                                        <Estrellas valor={Math.round(b.promedio)} />
                                        <span className="text-gold font-semibold w-10 text-right">{b.promedio.toFixed(1)}</span>
                                        <span className="text-gray-500 text-xs w-16 text-right">{b.cantidad} reseña{b.cantidad !== 1 ? "s" : ""}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {valoraciones.length === 0 ? (
                        <div className="bg-ink-card border border-line rounded-2xl p-12 text-center">
                            ...
                            <p className="text-gray-600 text-sm mt-1">Registrá la primera con "Nueva valoración"</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {valoraciones.map((v) => (
                                <div key={v.id_valoracion} className="bg-ink-card border border-line rounded-2xl p-5">
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-1">
                                                <Estrellas valor={v.estrellas} />
                                                <span className="text-white font-medium">{nombreBarbero(v.id_barbero)}</span>
                                            </div>
                                            <div className="text-xs text-gray-500 mb-2">
                                                Cliente: {nombreCliente(v.id_cliente)} · {formatoFecha(v.fecha_creacion)}
                                            </div>
                                            {v.comentario && (
                                                <p className="text-gray-300 text-sm">"{v.comentario}"</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                ...
                <>
                    ...
                    <div className="fixed top-0 right-0 h-full w-full max-w-md bg-ink-card border-l border-line z-50 overflow-y-auto">
                        ...
                        <form onSubmit={registrar} className="p-5 space-y-5">
                            ...
                            <div>
                                <label className="block text-sm text-gray-300 mb-1.5">Turno a valorar *</label>
                                    ...
                                    >
                                        ...
                                        {turnosSinValorar.map((t) => (
                                            <option key={t.id_turno} value={t.id_turno}>
                                                {t.fecha} · {nombreCliente(t.id_cliente)} · {nombreBarbero(t.id_barbero)}
                                            </option>
                                        ))}
                                    </select>
                                )}
                            ...
                            {turnoElegido && (
                                <div className="bg-ink rounded-lg border border-line px-4 py-3 text-sm">
                                    <div className="text-gray-400">Barbero: <span className="text-white">{nombreBarbero(turnoElegido.id_barbero)}</span></div>
                                    <div className="text-gray-400">Cliente: <span className="text-white">{nombreCliente(turnoElegido.id_cliente)}</span></div>
                                </div>
                            )}

                            {/* Estrellas clickeables */}
                            <div>
                                <label className="block text-sm text-gray-300 mb-2">Calificación *</label>
                                <div className="flex items-center gap-2">
                                    {[1, 2, 3, 4, 5].map((n) => (
                                        ...
                                        >
                                            <Star className={`w-8 h-8 ${n <= form.estrellas ? "text-gold fill-gold" : "text-gray-700"}`} />
                                        </button>
                                    ))}
                                    {form.estrellas > 0 && (
                                        <span className="text-gold font-semibold ml-2">{form.estrellas}/5</span>
                                    )}
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm text-gray-300 mb-1.5">Comentario</label>
                                <textarea
