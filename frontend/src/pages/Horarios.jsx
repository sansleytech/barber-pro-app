...
function Horarios() {
    ...
    const cambiarCampo = (idHorario, campo, valor) => {
        setHorarios((prev) =>
            prev.map((h) => (h.id_horario === idHorario ? { ...h, [campo]: valor } : h))
        );
    ...
    return (
        <div>
            ...
            <p className="text-gray-400 mb-6">Configurá los días y horas que trabaja cada barbero</p>
                    ...
                    {barberos.map((b) => {
                        ...
                        return (
                            <button
                                ...
                                className={`inline-flex items-center gap-2 rounded-full pl-1.5 pr-4 py-1.5 border transition-colors ${activo ? "bg-gold/10 border-gold text-gold" : "bg-ink border-line text-gray-300 hover:text-white"
                                    }`}
                            >
                                <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${activo ? "bg-gold text-ink" : "bg-ink-soft text-gray-400"
                                    }`}>
                                    {iniciales}
                                </span>
                                {b.nombre} {b.apellido}
            ...
            {barberoSel && (
                <>
                    ...
                    <p className="text-gray-400 text-sm mb-5">Activá los días que trabaja y definí horarios y pausas</p>
                </>
            )}
                ...
                <div className="space-y-3">
                    {DIAS.map((dia) => {
                        ...
                        return (
                            <div
                                key={dia.num}
                                className={`rounded-2xl border p-5 transition-colors ${activo ? "bg-ink-card border-gold/40" : "bg-ink-card/50 border-line"
                                    }`}
                            >
                                <div className="flex flex-wrap items-center gap-4">
                                    <span className={`font-semibold w-24 ${activo ? "text-gold" : "text-gray-500"}`}>
                                        {dia.nombre}
                                    </span>

                                    <button
                                        onClick={() =>
                                            activo ? desactivarDia(h.id_horario, dia.num) : activarDia(dia.num)
                                        }
                                        disabled={ocupado}
                                        className={`relative w-12 h-6 rounded-full transition-colors disabled:opacity-50 ${activo ? "bg-gold" : "bg-gray-600"
                                            }`}
                                    >
                                        <span
                                            className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-all ${activo ? "left-6" : "left-0.5"
                                                }`}
                                        />
                                    </button>
                                    ...
                                    {activo ? (
                                        <>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    onChange={(e) => cambiarCampo(h.id_horario, "hora_inicio", e.target.value)}
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => cambiarCampo(h.id_horario, "hora_fin", e.target.value)}
                                                    className="bg-ink border border-line rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-gold"
                                                />
                                            </div>
                                            <span className="text-xs font-medium text-emerald-400 uppercase">Activo</span>
                                        ...
                                        <span className="text-xs font-medium text-gray-500 uppercase">Sin trabajar</span>
                                    )}
                                </div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-line/60">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                onChange={(e) => cambiarCampo(h.id_horario, "pausa_inicio", e.target.value)}
                                                className="bg-ink border border-line rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-gold"
                                            />
                                            <span className="text-gray-500 text-sm">a</span>
                                            <input
                                                ...
                                                onChange={(e) => cambiarCampo(h.id_horario, "pausa_fin", e.target.value)}
                                                className="bg-ink border border-line rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-gold"
                                            />
                                        </div>
                                        <span className="text-xs text-purple-400/70">Almuerzo / descanso (opcional)</span>
                                    </div>
                                )}
                            </div>
                        );
