...
import { Plus, Search, ShieldCheck, Pencil, Power, Trash2, Link2 } from "lucide-react";
...
function Usuarios() {
    ...
    const cambiarEstado = async (u) => {
        ...
        if (!confirm(`¿Seguro que querés ${accion} a "${u.nombre_usuario}"?`)) return;
        try {
    ...
    const eliminarUsuario = async (u) => {
        if (!confirm(`¿Seguro que querés eliminar a "${u.nombre_usuario}"?`)) return;
        try {
    ...
    const filtrados = usuarios.filter((u) =>
        `${u.nombre_usuario} ${u.rol}`.toLowerCase().includes(busqueda.toLowerCase())
    );
    ...
    const columnas = [
        {
            ...
            render: (u) => (
                <div className="flex items-center gap-2">
                    ...
                    {u.super_admin && <ShieldCheck className="w-4 h-4 text-gold" title="Super admin" />}
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            titulo: "Rol",
            render: (u) => (
                <span className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full capitalize ${badgeRol(u.rol)}`}>
                    {u.rol}
                </span>
            ),
        },
        {
            ...
            render: (u) => (
                <span className={`inline-flex items-center gap-1.5 text-sm ${u.activo ? "text-emerald-400" : "text-gray-500"}`}>
                    <span className={`w-2 h-2 rounded-full ${u.activo ? "bg-emerald-400" : "bg-gray-500"}`} />
        ...
        },
        {
            ...
            render: (u) => <span className="text-gray-400 text-sm">{formatoFecha(u.ultimo_acceso)}</span>,
        },
        {
            campo: "acciones",
            titulo: "Acciones",
                ...
                return (
                    <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                            ...
                            disabled={bloqueado}
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-30 disabled:cursor-not-allowed ${u.activo ? "bg-emerald-500" : "bg-gray-600"
                                }`}
                            title={u.activo ? "Desactivar" : "Activar"}
                        >
                            <span
                                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${u.activo ? "translate-x-6" : "translate-x-1"
                                    }`}
                            />
                        </button>
                        <button
        ...
        <div>
            ...
            {cargando ? (
                ...
                <Tabla
                    columnas={columnas}
                    datos={filtrados}
                    vacioTexto={busqueda ? "No se encontraron usuarios" : "Todavía no hay usuarios"}
                />
            )}
        </div>
    );
