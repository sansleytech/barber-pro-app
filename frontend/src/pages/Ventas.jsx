...
import {
    Plus, Search, BarChart3, Wallet, Package, Users, FileDown, Sheet,
} from "lucide-react";
...
function Ventas() {
    ...
    const textoMetodo = (m) => {
        const t = { efectivo: "Efectivo", tarjeta: "Tarjeta", transferencia: "Transferencia", nequi: "Nequi", daviplata: "Daviplata" };
        return t[m] || m;
    };
    ...
    const filtradas = ventas.filter((v) => {
        ...
        const coincideMetodo = metodoFiltro === "todos" || v.metodo_pago === metodoFiltro;
        const coincideDesde = !desde || fechaV >= desde;
        const coincideHasta = !hasta || fechaV <= hasta;
        const texto = `${nombreCliente(v.id_cliente)} ${v.numero_factura || ""} ${v.id_venta}`.toLowerCase();
    ...
    const metricas = [
        { titulo: "VENTAS HOY", valor: ventasHoy.length, sub: "transacciones", icono: BarChart3, color: "text-white", borde: "border-line" },
        { titulo: "TOTAL HOY", valor: formatoPrecio(totalHoy), sub: "incluye IVA si aplica", icono: Wallet, color: "text-gold", borde: "border-gold/30" },
        { titulo: "PRODUCTOS", valor: unidadesHoy, sub: "unidades vendidas hoy", icono: Package, color: "text-white", borde: "border-line" },
        { titulo: "CLIENTES ÚNICOS", valor: clientesUnicosHoy, sub: "hoy", icono: Users, color: "text-white", borde: "border-line" },
    ];
    ...
    const exportarPDF = () => {
        ...
        doc.setFontSize(11);
        doc.setTextColor(0);
        doc.text(`Total: ${formatoPrecio(totalGeneral)}`, 14, doc.lastAutoTable.finalY + 10);
    ...
    const columnas = [
        {
            ...
            render: (v) => (
                <div>
                    <div className="text-white font-medium">#{v.numero_factura || v.id_venta}</div>
                    <div className="text-xs text-gray-500">{formatoFechaHora(v.fecha_venta)}</div>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            titulo: "Total",
            alinear: "right",
            render: (v) => <span className="text-gold font-semibold">{formatoPrecio(v.total)}</span>,
        },
    ];
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-white mb-1">Ventas de productos</h1>
            ...
            {cargando ? (
                <div className="text-center py-16 text-gray-500">Cargando ventas...</div>
            ) : error ? (
                ...
                <Tabla
                    columnas={columnas}
                    datos={filtradas}
                    vacioTexto={ventas.length === 0 ? 'Sin ventas. Hacé tu primera venta con "Nueva venta"' : "No hay ventas con esos filtros"}
                />
            )}
        </div>
    );
