...
function Categorias() {
  ...
  const columnas = [
    ...
    },
    {
      ...
      render: (c) => <span className="text-gray-400 text-sm">{c.color || "—"}</span>,
    },
    {
      campo: "orden",
      titulo: "Orden",
  ...
  return (
    <div>
      ...
      {cargando ? (
        <div className="text-center py-16 text-gray-500">Cargando categorías...</div>
      ) : error ? (
        ...
        <Tabla
          columnas={columnas}
          datos={filtrados}
          vacioTexto={busqueda ? "No se encontraron categorías" : "Todavía no hay categorías"}
        />
      )}
    </div>
  );
