...
import {
  Store, Receipt, Share2, MessageSquare, Save, Check, Plus, Trash2,
  Globe, MessageCircle, Link as LinkIcon,
} from "lucide-react";
...
function Configuracion() {
  ...
  const infoRed = (tipo) => TIPOS_RED.find((t) => t.tipo === tipo) || TIPOS_RED[6];
  ...
  const hayCambios =
    Object.keys(valores).some((clave) => valores[clave] !== (original[clave] ?? "")) ||
  ...
  const inputClase = "w-full bg-ink border border-line rounded-lg px-4 py-2.5 text-white placeholder-gray-600 focus:outline-none focus:border-gold";
  ...
  return (
    <div className="max-w-3xl">
          ...
          return (
            <button
              ...
              className={`flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                tab === t.id ? "bg-gold text-ink" : "text-gray-400 hover:text-white"
              }`}
            >
      ...
      <div className="bg-ink-card border border-line rounded-2xl p-6 mb-6">
        ...
        {tab === "negocio" && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-300 mb-1.5">Nombre de la barbería</label>
              <input type="text" value={valores.negocio_nombre ?? ""} onChange={(e) => cambiar("negocio_nombre", e.target.value)} placeholder="Barber Pro" className={inputClase} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">Teléfono</label>
                <input type="text" value={valores.negocio_telefono ?? ""} onChange={(e) => cambiar("negocio_telefono", e.target.value)} placeholder="300 000 0000" className={inputClase} />
              </div>
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">Email</label>
                <input type="text" value={valores.negocio_email ?? ""} onChange={(e) => cambiar("negocio_email", e.target.value)} placeholder="contacto@barberpro.com" className={inputClase} />
              </div>
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">Dirección</label>
                <input type="text" value={valores.negocio_direccion ?? ""} onChange={(e) => cambiar("negocio_direccion", e.target.value)} placeholder="Calle 00 #00-00" className={inputClase} />
              </div>
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">Ciudad</label>
                <input type="text" value={valores.negocio_ciudad ?? ""} onChange={(e) => cambiar("negocio_ciudad", e.target.value)} placeholder="Medellín" className={inputClase} />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">NIT</label>
                <input type="text" value={valores.negocio_nit ?? ""} onChange={(e) => cambiar("negocio_nit", e.target.value)} placeholder="900123456-7" className={inputClase} />
              </div>
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">Razón social</label>
                <input type="text" value={valores.negocio_razon_social ?? ""} onChange={(e) => cambiar("negocio_razon_social", e.target.value)} placeholder="Inversiones Barber SAS" className={inputClase} />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-gray-500">Si está activo, las ventas calculan IVA</div>
              </div>
              <button
                type="button"
                onClick={() => cambiar("aplica_iva", valores.aplica_iva === "true" ? "false" : "true")}
                className={`relative w-12 h-6 rounded-full transition-colors ${valores.aplica_iva === "true" ? "bg-gold" : "bg-line"}`}
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform ${valores.aplica_iva === "true" ? "translate-x-6" : ""}`} />
              </button>
            </div>

            {valores.aplica_iva === "true" && (
              <div>
                <label className="block text-sm text-gray-300 mb-1.5">Porcentaje de IVA (%)</label>
                <input
                  type="text"
                  inputMode="numeric"
                  value={valores.porcentaje_iva ?? ""}
                  onChange={(e) => cambiar("porcentaje_iva", e.target.value.replace(/[^0-9.]/g, ""))}
                  placeholder="19"
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1.5">Moneda</label>
              <select
        ...
        {tab === "redes" && (
          <div className="space-y-5">
            <p className="text-sm text-gray-400">Agregá las redes que quieras con su link.</p>
            ...
            {redes.length > 0 && (
              <div className="space-y-2">
                {redes.map((r, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 bg-ink border border-line rounded-lg px-4 py-3">
                      <LinkIcon className={`w-5 h-5 ${info.color}`} />
                      <div className="flex-1 min-w-0">
                        <div className="text-white text-sm font-medium">{info.nombre}</div>
                        <a href={r.url} target="_blank" rel="noreferrer" className="text-xs text-gray-500 hover:text-gold truncate block">{r.url}</a>
                      </div>
                      <button onClick={() => quitarRed(i)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
            ...
            <div className="border-t border-line pt-4">
              <label className="block text-sm text-gray-300 mb-2">Agregar red</label>
              <div className="flex flex-col sm:flex-row gap-2">
                <select value={nuevaRedTipo} onChange={(e) => setNuevaRedTipo(e.target.value)} className={`${inputClase} sm:max-w-[160px]`}>
                  {TIPOS_RED.map((t) => (
                ...
                />
                <button onClick={agregarRed} className="inline-flex items-center justify-center gap-1 bg-gold text-ink font-semibold rounded-lg px-4 py-2.5 hover:bg-gold-soft transition-colors">
        ...
        {tab === "mensajes" && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-300 mb-1.5">Pie de comprobante</label>
              <textarea value={valores.msg_pie_comprobante ?? ""} onChange={(e) => cambiar("msg_pie_comprobante", e.target.value)} rows={3} placeholder="¡Gracias por su visita!" className={inputClase} />
            </div>
            <div>
              <label className="block text-sm text-gray-300 mb-1.5">Mensaje de confirmación de turno</label>
              <textarea value={valores.msg_confirmacion_turno ?? ""} onChange={(e) => cambiar("msg_confirmacion_turno", e.target.value)} rows={3} placeholder="Le confirmamos su turno..." className={inputClase} />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="sticky bottom-0 bg-ink/80 backdrop-blur py-4 flex items-center gap-3">
        ...
        {hayCambios && !guardadoOk && <span className="text-gray-500 text-sm">Tenés cambios sin guardar</span>}
      </div>
    </div>
  );
}
