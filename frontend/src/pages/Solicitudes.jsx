    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-400">
          Pedidos de turno desde el portal público
        </p>
      </div>
        ...
        {FILTROS.map((f) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              filtro === f.valor
                ? "bg-gold text-ink"
                : "bg-ink-card text-gray-400 hover:text-white border border-line"
            }`}
          >
            {f.label}
          </button>
          ...
          {solicitudes.map((s) => (
            ...
            >
              ...
              <div className="space-y-2 text-sm text-gray-300 mb-4">
                <div className="flex items-center gap-2">
                  <Phone className="w-4 h-4 text-gray-500" /> {s.telefono}
                </div>
                <div className="flex items-center gap-2">
                  <Scissors className="w-4 h-4 text-gray-500" />{" "}
                  {nombreBarbero(s.id_barbero)}
                </div>
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-gray-500" />
                  {formatFecha(s.fecha_preferida) || "Sin día"} ·{" "}
                  {FRANJAS[s.franja_preferida] || s.franja_preferida}
                </div>
                {s.comentario && (
                  <div className="flex items-start gap-2">
                    <MessageSquare className="w-4 h-4 text-gray-500 mt-0.5" />{" "}
                    <span className="text-gray-400">{s.comentario}</span>
                  </div>
                )}
              </div>
              ...
              {s.estado === "pendiente" && (
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => cambiarEstado(s.id_solicitud, "atendida")}
                  ...
                  <button
                    onClick={() => cambiarEstado(s.id_solicitud, "descartada")}
