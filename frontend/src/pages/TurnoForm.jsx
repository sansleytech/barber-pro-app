...
import { ArrowLeft, Save, Clock, DollarSign, Search, X, Plus, ChevronDown } from "lucide-react";
...
function TurnoForm() {
    ...
    const serviciosElegidos = servicios.filter((s) => form.ids_servicios.includes(s.id_servicio));
    const precioTotal = serviciosElegidos.reduce((acc, s) => acc + Number(s.precio), 0);
    ...
    return (
        <div className="w-full">
            ...
            <p className="text-gray-400 mb-6">
                {editando ? "Modificá los datos del turno" : "Agendá una cita para tu barbería"}
            </p>
            ...
            <form onSubmit={enviar} className="bg-ink-card border border-line rounded-2xl p-6 md:p-8 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="sm:col-span-2">
                        <label className="block text-sm text-gray-300 mb-1.5">Cliente *</label>
                        <select
                ...
                <div>
                    <label className="block text-sm text-gray-300 mb-2">Servicios *</label>
                    ...
                    <div className="relative">
                        ...
                        >
                            <ChevronDown className={`w-5 h-5 transition-transform ${listaAbierta ? "rotate-180" : ""}`} />
                        </button>
                    </div>
                        ...
                        <div className="mt-2 border border-line rounded-lg overflow-hidden divide-y divide-line max-h-60 overflow-y-auto">
                            {serviciosFiltrados.length === 0 ? (
                                <p className="text-sm text-gray-500 px-4 py-3">
                                    {servicios.length === 0 ? "No hay servicios cargados" : "No hay más servicios para agregar"}
                                </p>
                            ) : (
                                serviciosFiltrados.map((s) => (
                                    ...
                                    >
                                        <div>
                                            <div className="text-white text-sm font-medium">{s.nombre}</div>
                ...
                {serviciosElegidos.length > 0 && (
                    <div className="flex items-center justify-between bg-ink rounded-lg border border-line px-4 py-3">
                        <span className="inline-flex items-center gap-2 text-gray-300 text-sm">
                            ...
                            Duración: <strong className="text-white">{duracionTotal} min</strong>
                        </span>
                        <span className="inline-flex items-center gap-2 text-gray-300 text-sm">
                            <DollarSign className="w-4 h-4 text-gold" />
                            Total: <strong className="text-gold">{formatoPrecio(precioTotal)}</strong>
                        </span>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3 border-t border-line pt-5">
                    ...
                    >
                        ...
                        {cargando ? "Guardando..." : editando ? "Guardar cambios" : "Crear turno"}
                    </button>
                    <button
                        type="button"
                        onClick={() => navigate("/turnos")}
